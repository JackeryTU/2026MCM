# -*- coding: utf-8 -*-
"""问题四：预测电价与实际结算价分离下的两种购电模式。

设计
----
* 价格基线四条：昨日同槽、近 7 日同槽均值、近 30 日同槽均值、近 30 日同槽中位数；
  四条共用同一条日内修正规则（最近至多 18 个已完成槽“实际价减 0 时基准”的算术
  均值，按 exp(-ell/3) 外推，下限 1e-4 元/kWh；0:00 无已完成槽时修正为 0），
  历史为空时回退附件 1 典型日价。
...
产物
----
results/result4-2.xlsx、results/result4-3.xlsx、results/csv/问题4_*.csv、
results/csv/问题4_门禁.json、results/csv/q4_bundle.npz、results/csv/q4_metrics.json。
"""
...
def price_error_table(data, offset=OFFSET, end=NTOT):
    """0:00 基准与 12:00 修正后两种口径的价格预测误差（元/kWh）。

    尖峰时段定义为当日真实电价不低于当日 75% 分位的槽（数据驱动，非固定钟点）。
    """
    A = data["price_act"]
...
def slot_table(run, price_act, day_indices):
    """表 1：代表性日六个指定槽的合同、执行与结算价（价格用真实结算价）。"""
    rows = []
