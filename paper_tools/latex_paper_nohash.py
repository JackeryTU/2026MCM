"""Adapted from the supplied latex_paper.py; NO HASHES.
All unrelated compilation, source, reference, typography and PDF checks retained.
Stat records cannot establish content identity. See README.md for provenance.
"""
...
SKILL_ROOT = Path(os.environ.get("MATH_MODELING_SKILL_ROOT", r"C:/Users/lenovo/.codex/skills/math-modeling-skill-main")).resolve()
TEMPLATE_ROOT = SKILL_ROOT / "tools" / "latex" / "assets" / "templates"
NOHASH_POLICY = "User explicitly prohibited all hash computation/comparison. Only path, size and mtime are checked; not content-integrity proof."
...
def _tree_metadata(path: Path) -> dict:
    if path.is_file():
        return _file_metadata(path)
    return {item.relative_to(path).as_posix(): _file_metadata(item)
            for item in sorted(path.rglob("*")) if item.is_file()}


def _file_metadata_map(path: Path) -> dict:
    if path.is_file():
        return {"main.tex": _file_metadata(path)}
    return _tree_metadata(path)
...
def prepare_project(
    ...
    *,
    ...
    manifest = {
        "schema_version": 2,
        "integrity_policy": NOHASH_POLICY,
        "contest": contest,
        ...
        "template": {
            ...
            "metadata": _tree_metadata(source),
        },
        "template_files": _file_metadata_map(source),
        "resource_bindings": [],
...
def _validated_resource_bindings(manifest: dict) -> list[dict]:
    ...
    for index, binding in enumerate(bindings):
        ...
        if not isinstance(binding.get("metadata"), dict) or not binding["metadata"]:
            raise ValueError("resource binding requires nonempty stat metadata")
        if binding.get("metadata_mode", "tree") not in {"content", "tree"}:
            raise ValueError(
                f"resource_bindings[{index}].metadata_mode 必须是 content 或 tree"
            )
    return bindings


def _resource_metadata(path: Path, mode: str) -> dict:
    if mode == "content":
        if not path.is_file():
            raise ValueError("文件元数据只能用于单文件资源")
        return _file_metadata(path)
    return _tree_metadata(path)
...
def _resource_binding_issues(root: Path) -> list[str]:
    ...
    for binding in bindings:
        try:
            ...
            metadata_mode = binding.get("metadata_mode", "tree")
            ...
            if not source.exists() or not destination.exists():
                ...
                _resource_metadata(source, metadata_mode) != expected
                or _resource_metadata(destination, metadata_mode) != expected
            ):
                issues.append(
    ...
    for path in sorted(root.rglob("*")):
        ...
        if any(relative == bound or bound in relative.parents for bound in bound_destinations):
            continue
        template_metadata = template_files.get(relative.as_posix())
        if template_metadata and _file_metadata(path) == template_metadata:
            continue
        issues.append(
...
def bind_resource(main_tex: Path, source_path: Path, destination: str) -> dict:
    ...
    metadata_mode = "content" if source.is_file() else "tree"
    source_metadata = _resource_metadata(source, metadata_mode)
    if _resource_metadata(target, metadata_mode) != source_metadata:
        raise ValueError("权威资源与 LaTeX 资源副本大小或修改时间不一致，不能建立元数据绑定")
...
def source_bundle_metadata(main_tex: Path) -> dict:
    ...
    for path in sorted(item for item in root.rglob("*" ) if item.is_file()):
        ...
        if path.name.endswith((".build.json", ".conversion.json")):
            continue
        metadata[relative.as_posix()] = _file_metadata(path)
    return metadata
...
def _verify_pdf_binding(
    main: Path, pdf: Path, source_metadata: dict
    ...
    if manifest.get("source_metadata") != source_metadata:
        issues.append("PDF 对应源码元数据与当前 LaTeX 项目不一致")
    if manifest.get("pdf_metadata") != _file_metadata(pdf):
        issues.append("PDF 文件元数据与构建清单不一致")
...
def inspect_paper(
    ...
    *,
    ...
    source_metadata = source_bundle_metadata(main)
    rendered_pages = None
    body_pages = None
    pdf_audit = None
    build_manifest = None
    ...
    if pdf is not None and pdf.is_file():
        build_manifest, binding_issues = _verify_pdf_binding(main, pdf, source_metadata)
...
def _tool_version(executable: str | None) -> str | None:
    ...
    # Windows latexmk may print console code-page diagnostics before its version.
    if name == "latexmk":
        for line in lines:
            if line.startswith("Latexmk,") and "Version" in line:
                return line
...
def build_paper(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="latex-build-") as temporary_directory:
        ...
        compiled_source_metadata = source_bundle_metadata(isolated_main)
        post_build_metadata = source_bundle_metadata(main)
        if compiled_source_metadata != source_metadata:
            fatal_issues.append("隔离编译过程修改了源码副本，拒绝发布")
        if post_build_metadata != source_metadata:
        ...
        "integrity_policy": NOHASH_POLICY,
            ...
            "project_root": str(root),
            "source_metadata": source_metadata,
            "compiled_source_metadata": compiled_source_metadata,
            "post_build_source_metadata": post_build_metadata,
            "pdf_metadata": _file_metadata(isolated_pdf),
            "project_manifest_metadata": (
                _file_metadata(project_manifest) if project_manifest.is_file() else None
            ),
            "engine": engine,
            "tools": {
                "latexmk": {
