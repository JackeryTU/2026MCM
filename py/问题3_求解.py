# -*- coding: utf-8 -*-
"""问题三：官方光伏预报档案、预见期分层风险储备、合同调整与滚动 LP 执行。
...
产物
----
results/result3.xlsx、results/csv/问题3_*.csv、results/csv/问题3_门禁.json，
并写出 results/csv/q3_bundle.npz + q3_metrics.json 供 make_figures.py 出图。

用法
----
python 问题3_求解.py                全量运行（并行配置扫描）
python 问题3_求解.py --days 45      最小可运行冒烟（只跑前 45 天）
python 问题3_求解.py --workers 8    指定并行进程数
python 问题3_求解.py --main-only    只运行主配置
"""
...
def _coverage(data, prof, allowed, mode, alpha, W, d0=31, d1=365):
    """预测安全轨迹对实测净负荷的覆盖率、Pinball 与残差 MAE（§11.4）。"""
...
def lead_scale_table(data, prof, offset=31):
    """附件3 预见期 1…24 h 的分来源误差与经验尺度 s(h)（§8.1）。"""
    rows = []
