...
def merge_positive_intervals(values: np.ndarray, *, tolerance: float = 1e-8) -> list[tuple[int, int, float]]:
    """Merge consecutive positive slots without crossing a daily boundary."""
    x = np.asarray(values, dtype=float)
    if x.shape != (N_SLOTS,):
        raise ValueError("应急电量必须是一日144槽")
...
def _interval_group_label(start: int, end: int) -> str:
    a, b = start * 10, end * 10
...
def _write_emergency_sheet(ws, results) -> None:
    ...
    for result in results:
        ...
        groups = merge_positive_intervals(emergency)
        if abs(sum(value for _, _, value in groups) - float(np.sum(emergency))) > 1e-7:
            raise RuntimeError("应急区间合并前后总量不一致")
        for start, end, value in groups:
            rows.append((result["date"].to_pydatetime(), _interval_group_label(start, end), value))
    if not rows:
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="result directory, relative to project root unless an absolute path is supplied",
    )
    parser.add_argument("--alpha", type=float, default=DEFAULT_ALPHA)
    parser.add_argument("--forecast-model", choices=("F0", "F1", "F2"), default="F1")
    parser.add_argument("--history-days", type=int, choices=(21, 30, 45, 60), default=DEFAULT_HISTORY_DAYS)
    parser.add_argument(
    ...
    results_dir = (requested_output if requested_output.is_absolute() else root / requested_output).resolve()
    if results_dir == root or root not in results_dir.parents:
        raise ValueError("--output-dir must resolve to a directory inside --project-root")
    results_dir.mkdir(parents=True, exist_ok=True)
    started = time.time()
    np.random.seed(SEED)

    data = load_inputs(root)
    forecast_archive = build_forecast_archive(data)
    selected_forecast = forecast_archive[args.forecast_model]
    load_point = selected_forecast["load_kw"]
    load_residual = selected_forecast["load_residual_kw"]
    pv_point = selected_forecast["pv_kw"]
    pv_residual = selected_forecast["pv_residual_kw"]
    ...
    q2_cache, q3_cache = precompute_safe_trajectories(
        ...
        args.alpha, args.risk_mode, args.history_days,
    )
    mpc_config = MpcConfig(
        horizon_slots=args.mpc_horizon,
        tracking_weight=args.tracking_weight,
    )

    q1 = run_q1(data)
    common = dict(
        load_point=load_point, pv_point=pv_point, load_residual=load_residual,
        pv_residual=pv_residual, issued_residual=issued_residual,
        q2_cache=q2_cache, q3_cache=q3_cache, alpha=args.alpha,
    ...
    write_representative_tables(root, variants, REPRESENTATIVE_DATES, results_dir)
    rep_rows = []
    for name, (variant_results, adjusted) in variants.items():
    ...
    metrics = {
        ...
        "mpc_horizon_slots": args.mpc_horizon,
        "tracking_weight": args.tracking_weight,
    ...
    manifest = {
        ...
        "seed": SEED,
        "parameters": {"alpha": args.alpha, "risk_mode": args.risk_mode, "controller": args.controller, "mpc_horizon_slots": args.mpc_horizon, "tracking_weight": args.tracking_weight, "eta_charge": 0.9, "eta_discharge": 0.9, "soc_min_kwh": 1200, "soc_max_kwh": 10800, "power_limit_kw": 5000},
    ...
    output_arg = results_dir.relative_to(root).as_posix()
    manifest["command"] = (
    ...
    (results_dir / "主运行记录.json").write_text(manifest_text, encoding="utf-8")
    (results_dir / "复现清单.json").write_text(manifest_text, encoding="utf-8")
    (results_dir / "manifest_nohash.json").write_text(manifest_text, encoding="utf-8")
