...
\begin{equation}
 \min\ C_1=\sum_{t=0}^{143}c_tg_t^{\mathrm P},
 \label{eq:q1objective}
\end{equation}
并在式\eqref{eq:plan-balance}--\eqref{eq:plan-bounds}基础上增加循环边界 $S_0^{\mathrm P}=S_{144}^{\mathrm P}=6000$。问题一没有应急购电机制，无须在LP中引入执行记账量 $u_t$。该模型是线性规划。由于同一最优费用下可能存在多种等价储能轨迹，先求得最小费用 $C_1^*$，再在
\begin{equation}
 C_1\le C_1^*+10^{-8}\max\{1,|C_1^*|\}
 \label{eq:q1tol}
\end{equation}
的容差内最小化 $\sum_t(x_t^{\mathrm P}+y_t^{\mathrm P})$。因此，报告的费用具有主目标最优性，而储能轨迹是低吞吐代表解，不宣称是唯一最优轨迹。

\subsection{循环恒等式与充放电互斥}
\label{sec:q1-mutual-exclusion}

将式\eqref{eq:plan-soc}在全天求和，并利用循环边界，可得
\begin{equation}
 \eta_c\sum_t x_t^{\mathrm P}=\frac{1}{\eta_d}\sum_t y_t^{\mathrm P},
 \qquad
 \sum_t y_t^{\mathrm P}=\eta_c\eta_d\sum_t x_t^{\mathrm P}.
 \label{eq:q1-cycle-identity}
\end{equation}
这说明放电总量小于充电总量并非能量不守恒，而是往返损耗的直接结果。本文没有增加0--1互斥变量：在电价非负、不可售电、允许弃光且没有吞吐补贴时，总能找到费用不增而吞吐不增的代表解；二阶段最小吞吐目标会从同费用解中排除无意义的同时充放。数值结果中144个时槽的同时充放量均为0，因此连续LP已经给出物理可执行的互斥轨迹。
